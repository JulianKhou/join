export function editTaskTemplate(task, categories, contacts) {
  let taskDetail = `
    <!-- Overlay -->
    ...
    <div class="overlay-edit-card" id="overlayEditCard-${task.id}">
      ...
      <div class="overlay-actions">
        <button class="btn-delete" id="deleteTaskBtn-${task.id}">
        ...
        <button class="btn-edit" id="editTaskBtn-${task.id}">
    ...
    <div class="edit-overlay-card" id="editOverlayCard-${task.id}">
        ...
        <div class="form-column-left">
          <div class="form-group">
            ...
            <input type="text" id="editTaskTitle-${
              task.id
            }" class="form-input" value="${task.title}" required>
          </div>

          <div class="form-group">
            <label for="editTaskDescription-${task.id}">Description</label>
            <textarea id="editTaskDescription-${
              task.id
            }" class="form-textarea">${task.description}</textarea>
          </div>

          <div class="form-group">
            <label>Assigned to</label>
            <div class="multi-select">
              <input class="selected-box task-input" id="editSelectedBox-${
                task.id
              }" placeholder="Select contacts to assign" readonly />
              <div class="checkbox-list" id="editCheckboxList-${task.id}"></div>
            </div>
            <div class="selected-badges" id="editAssignees-${task.id}"></div>
          </div>
        </div>
        ...
        <div class="form-column-right">
          ...
          <div class="form-group">
            ...
            <div class="priority-button-group">
              <button type="button" class="priority-button" id="edit-priority-urgent-${
                ...
                <svg class="priority-icon priority-button-icon-urgent" xmlns="http://www.w3.org/2000/svg" width="20" height="15" viewBox="0 0 20 15" fill="none">
              ...
              <button type="button" class="priority-button" id="edit-priority-medium-${
                task.id
              }" data-priority="Medium">
                <span class="priority-name">Medium</span>
                <svg class="priority-icon priority-button-icon-medium" xmlns="http://www.w3.org/2000/svg" width="20" height="8" viewBox="0 0 20 8" fill="none">
          ...
          <div class="form-group">
            ...
            <div class="subtask-input-group">
              <input type="text" id="editSubtasks-${
                task.id
              }" class="form-input" placeholder="Add new subtask">
              <div class="subtask-btn-group">
                <button type="button" class="subtask-action-btn" id="editAddSubtaskBtn-${
                  task.id
                }">
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none">
                    <path d="M5 12h14M12 5v14" stroke="#2A3647" stroke-width="2" stroke-linecap="round" />
                  </svg>
                </button>
                <div class="divider-line"></div>
                <button type="button" class="subtask-action-btn" id="editRemoveSubtaskBtn-${
                  task.id
                }">
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none">
                    <path d="M18 6L6 18M6 6L18 18" stroke="#2A3647" stroke-width="2" stroke-linecap="round" />
            ...
            <div class="edit-subtasks-list" id="editSubtasksList-${
              task.id
            }"></div>
          </div>
        </div>
      ...
      <div class="edit-overlay-actions">
        <button type="submit" form="editTaskForm-${
          task.id
        }" class="btn-create" id="editBtnOk-${task.id}">
          Ok <img src="./assets/utilitys/check.svg" alt="ok-icon">
        </button>
      </div>
    </div>
  `;
