    ...
    <div class="task-card grabbable" id="task-card-${tasks.id}" draggable="true">
          <div class="tasks-card-content">
            ...
            <div class="task-priority">
                <svg class="priority-icon priority-${tasks.priority.toLowerCase()} " xmlns="http://www.w3.org/2000/svg" width="17" height="7"
                  viewBox="0 0 17 7" fill="none">
                  <path
                    d="M16.0685 6.33333L0.931507 6.33333C0.684456 6.33333 0.447523 6.23448 0.272832 6.05852C0.0981406 5.88256 0 5.6439 0 5.39506C0 5.14621 0.0981406 4.90756 0.272832 4.7316C0.447523 4.55564 0.684456 4.45679 0.931507 4
            </div>
          </div>
        </div>` ;
    return taskCards;
...
export function editTaskFormTemplate(task, categories, contacts) {
  let taskEditForm = `
    <div class="edit-overlay-card" id="editOverlayCard-${task.id}">
      <!-- Close Button -->
      <button class="edit-overlay-close-btn" id="editOverlayCloseBtn">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
          <path d="M18 6L6 18M6 6L18 18" stroke="#2A3647" stroke-width="2" stroke-linecap="round"
            stroke-linejoin="round" />
        </svg>
      ...
      <!-- Form Content -->
      <form class="edit-task-form" id="editTaskForm-${task.id}">
        <!-- Title -->
        <div class="edit-form-group">
          <label for="editTaskTitle-${task.id}">Title</label>
          <input type="text" id="editTaskTitle-${task.id}" class="edit-form-input" value="${task.title}" required>
        </div>

        <!-- Description -->
        <div class="edit-form-group">
          <label for="editTaskDescription-${task.id}">Description</label>
          <textarea id="editTaskDescription-${task.id}" class="edit-form-textarea">${task.description}</textarea>
        </div>

        <!-- Due Date -->
        <div class="edit-form-group">
          <label for="editTaskDate-${task.id}">Due date</label>
        ...
        <div class="edit-form-group">
          ...
          <div class="edit-priority-group">
            ...
            <button type="button" class="edit-priority-btn" id="edit-priority-medium-${task.id}" data-priority="Medium">
              <span class="priority-name">Medium</span>
              <svg class="priority-icon priority-button-icon-medium" xmlns="http://www.w3.org/2000/svg" width="20"
                height="8" viewBox="0 0 20 8" fill="none">
                <path
            ...
            <button type="button" class="edit-priority-btn" id="edit-priority-low-${task.id}" data-priority="Low">
              <span class="priority-name">Low</span>
              <svg class="priority-icon priority-button-icon-low" xmlns="http://www.w3.org/2000/svg" width="20"
                height="15" viewBox="0 0 20 15" fill="none">
                <path
      ...
      <div class="edit-overlay-actions">
        <button type="submit" form="editTaskForm-${task.id}" class="edit-btn-ok" id="editBtnOk-${task.id}">Ok
